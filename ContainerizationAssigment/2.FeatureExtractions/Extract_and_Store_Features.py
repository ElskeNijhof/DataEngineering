...
@app.route('/FeatureExtractions/<table_name>', methods=['POST'])
...
@app.route('/FeatureExtractions/<table_name>', methods=['DELETE'])
def delete_table(table_name):
    db_util.drop_tb(table_name)
    return json.dumps({'message': 'a table was dropped'}, sort_keys=False, indent=4), 200


@app.route('/FeatureExtractions/<table_name>', methods=['PUT'])
...
@app.route('/FeatureExtractions/<table_name>', methods=['GET'])
